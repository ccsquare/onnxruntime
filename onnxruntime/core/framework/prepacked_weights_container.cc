...
AllocatorPtr PrepackedWeightsContainer::GetOrCreateAllocator(const std::string_view device_name) {
  auto iter = allocators_.find(std::string(device_name));
  ...
  if (device_name == CPU) {
    ...
    allocators_[std::string(device_name)] = allocator;
