...
def test_debug_options_save_onnx_models(mode):

    device = 'cuda'
    N, D_in, H, D_out = 64, 784, 500, 10
    # Create a temporary directory for the onnx_models
    with tempfile.TemporaryDirectory() as temporary_dir:
        os.environ['ORTModuleSaveONNXDirectory'] = temporary_dir
        model = NeuralNetSinglePositionalArgument(D_in, H, D_out).to(device)
        ort_model = ORTModule(model, DebugOptions(save_onnx=True, save_onnx_prefix='my_model'))
        if mode == 'inference':
            ort_model.eval()
        x = torch.randn(N, D_in, device=device)
        _ = ort_model(x)
...
def test_debug_options_save_onnx_models_validate_fail_on_non_writable_dir():
    ...
    os.environ['ORTModuleSaveONNXDirectory'] = '/non/existent/directory'
    with pytest.raises(Exception) as ex_info:
        _ = DebugOptions(save_onnx=True, save_onnx_prefix='my_model')
    assert "Directory /non/existent/directory is not writable." in str(ex_info.value)
    del os.environ['ORTModuleSaveONNXDirectory']

def test_debug_options_save_onnx_models_validate_fail_on_non_str_prefix():
    prefix = 23
    with pytest.raises(Exception) as ex_info:
        _ = DebugOptions(save_onnx=True, save_onnx_prefix=prefix)
...
def test_debug_options_save_onnx_models_validate_fail_on_no_prefix():
    ...
    assert f"save_onnx_prefix must be provided when save_onnx is set." in str(ex_info.value)
...
def test_debug_options_log_level_os_environment():
    ...
    os.environ['ORTModuleLogLevel'] = 'INFO'
    device = 'cuda'
    N, D_in, H, D_out = 64, 784, 500, 10
    model = NeuralNetSinglePositionalArgument(D_in, H, D_out).to(device)
    ort_model = ORTModule(model)
    ...
    del os.environ['ORTModuleLogLevel']
